...
class effect : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "SimpleFilt.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("SimpleFilt");
		ui_interface->addHorizontalSlider("freq", &fHslider0, 1000.0f, 10.0f, 10000.0f, 1.0f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow4 = (2.0f * (1.0f - (1.0f / effect_faustpower2_f(fSlow0))));
		for (int i = 0; (i < count); i = (i + 1)) {
			fRec0[0] = (float(input0[i]) - (fSlow2 * ((fSlow3 * fRec0[2]) + (fSlow4 * fRec0[1]))));
